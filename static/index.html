                ...
                <div className={`h-full flex flex-col ${isSakura ? 'bg-gradient-sakura' : 'bg-gradient-sensei'}`}>
                        ...
                        {!ready ? <Loading text="Caricamento..." /> : messages.length === 0 ? (
                            <div className="h-full flex items-center justify-center">
                                <div className="text-center max-w-xs">
                                    ...
                                    <p className="text-[var(--text-muted)] text-sm">{isSakura ? 'Ciao! Parliamo di mente, stress e benessere.' : 'Ciao! Il tuo coach per performance e longevità.'}</p>
                                </div>
                            </div>
                        ) : (
                    ...
                    <div className="flex-shrink-0 p-3 border-t border-[var(--border)] bg-[var(--bg-primary)]" style={{ paddingBottom: 'max(12px, calc(var(--safe-bottom) + 76px))' }}>
                        <div className="max-w-4xl mx-auto flex gap-2">
                            ...
                            <button onClick={() => send()} disabled={loading || !input.trim()} className={`${isSakura ? 'btn-pink' : 'btn-primary'} w-11 h-11 rounded-xl text-lg disabled:opacity-40 flex items-center justify-center`}>➤</button>
                        </div>
                    </div>
                </div>
            );
